...
export default function Shiden({ provider, account, connectWallet }) {
  ...
  const MAIN_WEBSITE_API = 'https://appsiko.com/api/get-token-data?network=shiden';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      try {
        ...
        console.log(`Using user config: wallet=${userWallet}, fee=${userFee} SDN`);
      } catch (error) {
  ...
  const handleViewToken = () => {
    if (contractAddress) {
      const etherscanUrl = `https://shiden.subscan.io/token/${contractAddress}`;
    ...
    <div className="container mx-auto p-6 max-w-3xl bg-background">
      ...
      <div className="mt-6 p-6 bg-muted rounded-xl animate-fadeIn">
        <h2 className="text-2xl font-bold font-sans text-black mb-4 relative">
          How to Create Your Token on the Shiden Blockchain
        ...
        <p className="text-black font-bold font-sans leading-relaxed mb-4">
          Follow These Simple Steps To Create Meme Coin At Shiden Within 10 - 30 Seconds:
        </p>
        <ol className="list-decimal list-inside text-black font-sans font-bold space-y-2">
          ...
          <li>
            Switch Your MetaMask Network to Shiden. If The Shiden Network Isn’t Showing On Metamask, You Can Add It As Your Custom Network. You Can Visit This Link For Getting RPC, Chain Id etc. Details <a href="https://chainlist.org/chain/336" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">https://chainlist.org/chain/336</a>, After Getting These Values You Can Add It As Custom Network.
          </li>
